...
def extract_response(json_line: dict, game: str, exptype: str):
    ...
    try:
        ...
        if not any(isinstance(i, dict) for i in response_dict.values()):
            ...
            if game == "dictator_sender" or game == "ultimatum_sender" or game == "dictator_sequential":
                ...
                if game == 'dictator_sequential':
                    ...
                    if exptype =='baseline':
                        fairness = re.findall(r"\d+", json_line[0]["messages"][0]["content"].split(".")[5])
                    else:
                        fairness = re.findall(r"\d+", json_line[0]["messages"][0]["content"].split(".")[6])
                ...
                if game == 'ultimatum_receiver':
                    ...
                    if exptype == 'baseline':
                        fairness = re.findall(r"\d+", json_line[0]["messages"][0]["content"].split(".")[7])
                    else:
                        fairness = re.findall(r"\d+", json_line[0]["messages"][0]["content"].split(".")[8])
                    ...
                    if exptype == 'baseline':
                        fairness = re.findall(r"\d+", json_line[0]["messages"][0]["content"].split(".")[3])
                    else:
                        fairness = re.findall(r"\d+", json_line[0]["messages"][0]["content"].split(".")[4])
        ...
        if json_line[1]["choices"][0]["finish_reason"] == 'content_filter':
            ...
            exception_list.append(json_line[0]["messages"][0]['content']) # the prompt
            exception_list.append(json_line[0]["temperature"]) # the model temperature
            exception_list.append(json_line[1]["choices"][0]["message"]["content"]) # the model response
