...
class AiParticipant:
    ...
    def adjust_prompts(self):
        ...
        if self.game.endswith('_sender'):
            ...
            baseline_df = pd.DataFrame(row_list_b, columns=list(["prompt", "temperature"]))
            ...
            for t in self.game_params['temperature']:
                for comb in all_comb_b:
                    ...
                    final_prompt = self.game_params[f"prompt_baseline"].format(**prompt_params_dict)
                    row = [final_prompt] + [t] + [prompt_params_dict[f] for f in factors_b]
                    row_list_b.append(row)
            baseline_df = pd.DataFrame(row_list_b, columns=list(["prompt", "temperature"] + factors_b))
        ...
        for t in self.game_params['temperature']:
            for comb in all_comb_e:
                ...
                final_prompt = self.game_params[f"prompt_complete"].format(**prompt_params_dict)
                row = [final_prompt] + [t] + [prompt_params_dict[f] for f in factors_e]
                row_list_e.append(row)

        experimental_df = pd.DataFrame(row_list_e, columns=list(["prompt", "temperature"] + factors_e))
        experimental_df['prompt_type'] = 'experimental'
        experimental_df['game_type'] = self.game
        # add number of tokens that the prompt corresponds to
        experimental_df['n_tokens_davinci'] = experimental_df['prompt'].apply(lambda x: self.count_num_tokens(x, 'davinci'))
        experimental_df['n_tokens_chatgpt'] = experimental_df['prompt'].apply(lambda x: self.count_num_tokens(x, 'chatgpt'))
    ...
    def collect_control_answers(self):
        ...
        control_schema = pa.DataFrameSchema({
            ...
            "control_answer": pa.Column(str, checks=[
        ...
        control_df.to_csv(os.path.join(prompts_dir, self.game, f'{self.game}_control_questions.csv'),
                          sep=',',
                          index=False)
